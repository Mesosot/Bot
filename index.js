...
const limitlessCalcbotCh = process.env.Channel;
...
client.on("message", (msg) => {
  if (
    !(
      msg.content.startsWith("Session data") &&
      msg.channel.id === limitlessCalcbotCh
    ) ||
    msg.author.bot
  )
    return;
  ...
  for (let i = 0; i <= ptmembers - 1; i++) {
    ...
    ptmemberbalance[i] = parseInt(
      lines[6 * (i + 1) + 3].trim().split(" ")[1].replace(/,/g, "")
    );
  ...
  while (first < last && sortedTbl[first][1] < avgr) {
    if (sortedTbl[last][1] > avgr) {
      ...
      if (minus >= avgr) {
        outcome.push(
          `**${sortedTbl[last][0]}** should give **${formatNumber(
            avgDiffer
          )}** gp to **${sortedTbl[first][0]}**`
        );
